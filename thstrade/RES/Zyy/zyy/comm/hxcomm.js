...
// 通过回调客户端的参数来获取userkey对应的资金账户
function GetZjzh(strUserKey)
{
	...
	return (fnFunction('get_zjzh_ex', strUserKey) || '');
}
 ...
 * function		: 特征匹配
 * Date			: 2017-10-11	
 * parameter	: 特征数组array 形如XXX*	，	待匹配str
 * return		: true 表示符合匹配规则， false则为不匹配
 * 说明			：先删除特征数组的最后一位*号后做比对，非XXX*形式请勿使用
 * **********************************************************************/
function MatchFeature(arrayFeature, strToMatch)
{	
	for (var i = 0 ; i < arrayFeature.length; i++ )
	{	
		if ( strToMatch.substring(0,arrayFeature[i].length - 1) == arrayFeature[i].substring(0,arrayFeature[i].length-1) )
		{
			return true;
		}
	}
 ...
 * function		: 计算请求耗时
 * Date			: 2017-12-09
 * parameter	: reqStart：请求开始时间戳
 * return		: reqTime：请求耗时
 * *****************************************************************/
function CalcReqTime(reqStart)
{
	var reqEnd = new Date().getTime();
	var reqTime = (reqEnd - reqStart) / 1000;	//毫秒转化为秒
